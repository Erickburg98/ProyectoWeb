 ...
 * @author darkdestiny
 */
@WebServlet(name = "LoginServletAlumno", urlPatterns = {"/LoginServletAlumno"})
public class LoginServletAlumno extends HttpServlet {

    private static final String SQL_SELECT = "{call LoginAlu(?)}";
    ...
    private void iniciarSesion(HttpServletRequest request, HttpServletResponse response) throws SQLException {
        ...
        String newAlumno = request.getParameter("newAlumno");
        String username = "Erickburg";
        String password = "Erickburg";

        if (newAlumno.equals("no")) {
        ...
        try {
            ...
            if (ok == true) {
                ...
                if (newAlumno.equals("si")) {
                    response.sendRedirect("AlumnoServlet?accion=nuevo");
                } else {
                    response.sendRedirect("AlumnoServlet?accion=listaDeAlumnos&id=" + idUser);
                }
